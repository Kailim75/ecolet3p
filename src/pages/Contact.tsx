    ...
    if (isSubmitted) {
      return (
        <motion.div className="text-center py-12" initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.5 }}>
          ...
          <p className="text-muted-foreground mb-2">Notre équipe vous recontacte sous 24h maximum.</p>
          <div className="bg-forest/5 rounded-xl p-4 mb-6 text-left max-w-sm mx-auto">
            <p className="text-sm font-semibold text-forest mb-2">🔜 Prochaines étapes :</p>
            <ul className="text-sm text-muted-foreground space-y-1.5">
              <li className="flex items-start gap-2"><CheckCircle className="w-4 h-4 text-forest shrink-0 mt-0.5" /> Confirmation par email</li>
              <li className="flex items-start gap-2"><CheckCircle className="w-4 h-4 text-forest shrink-0 mt-0.5" /> Un conseiller vous appelle</li>
              <li className="flex items-start gap-2"><CheckCircle className="w-4 h-4 text-forest shrink-0 mt-0.5" /> Inscription finalisée ensemble</li>
            </ul>
          </div>
          <p className="text-sm text-muted-foreground mb-4">Besoin d'une réponse immédiate ?</p>
          <a href="https://wa.me/33783787663?text=Bonjour%2C%20je%20viens%20d%27envoyer%20une%20demande%20sur%20le%20site" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 bg-[#25D366] text-white font-bold py-2.5 px-5 rounded-lg hover:bg-[#20BD5A] transition-colors text-sm mb-4">
            WhatsApp — Réponse rapide
          </a>
          <br />
          <button onClick={() => { setIsSubmitted(false); setStep(1); setFormData({ fullName: "", email: "", phone: "", formation: "", message: "", consent: false }); }} className="text-sm text-muted-foreground hover:text-forest transition-colors underline">Envoyer une autre demande</button>
        </motion.div>
      );
    }
